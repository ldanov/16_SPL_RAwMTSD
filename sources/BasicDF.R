basic.DF.old = function(data, fDcols = colnames(data)) {
    ...
    foreach(i = 1:length(fDcols)) %do% {
        ...
        out.ur = c(fDcols[i], "UR.base", summary(ur)$coefficients[1], summary(ur)$coefficients[3], pf(summary(ur)$fstatistic[1], 
            summary(ur)$fstatistic[2], summary(ur)$fstatistic[3], lower.tail = F))
        out.ur2 = c(fDcols[i], "UR.drift", summary(ur.dr)$coefficients[2], summary(ur.dr)$coefficients[6], pf(summary(ur.dr)$fstatistic[1], 
            summary(ur.dr)$fstatistic[2], summary(ur.dr)$fstatistic[3], lower.tail = F))
        out.ur3 = c(fDcols[i], "UR.drift.tTrend", summary(ur.dr.tt)$coefficients[2], summary(ur.dr.tt)$coefficients[8], 
            pf(summary(ur.dr.tt)$fstatistic[1], summary(ur.dr.tt)$fstatistic[2], summary(ur.dr.tt)$fstatistic[3], 
                lower.tail = F))
        
        
        # temp = list(ur, ur.dd, ur.dd.tt) names(temp) = c(paste0(fDcols[i], '_DF_ur'), paste0(fDcols[i], '_DF_urdr'),
